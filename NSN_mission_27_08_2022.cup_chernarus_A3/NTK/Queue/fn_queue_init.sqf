/*
Falta cartelito que te avise antes de hacerte pija 
*/
...
addMissionEventHandler ["PlayerConnected", {
	...
	if ( missionNamespace GetVariable ["NSN_VAR_LIMIT_RECONECTIONS",true] ) then {
		_reconexiones_actuales = nsn_var_reconneciones getOrDefault [_uid,0];
		...
		if ( _reconexiones_actuales > (missionNamespace getvariable ["NSN_VAR_MAX_RECONECTIONS",4]) ) then {
			_serverCommandPw serverCommand (format ["#exec ban ""%1"" ", _uid ]);
	...
	/* 
	FALTA TESTEAR LA PARTE DE LLEGADA TARDE  
	//tiempo limite 
